...
## Codebase Intelligence for hp-site (Repowise)

Indexed by [Repowise](https://repowise.dev). Last indexed: 2026-07-21 (commit ba0c425). Confidence: 100%.
...
- `src/content/mirrors/index.ts`

### Files that need care (bug-fix history first, then churn — check `get_risk` before editing)
- `src/app/layout.tsx` — 3 commits/90d

### Code health
Three co-equal signals: defect risk 9.76/10 avg, hotspot health 9.65/10 (stable), worst `src/components/sections/hero.tsx` at 9.62/10 · maintainability 9.53/10 · performance risk 1 open static I/O-in-loop / N+1 finding. Detail: `get_health()`.
